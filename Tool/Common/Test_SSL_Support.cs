...
public class Test_SSL_Support
{
		...
		for (int i = 0; i < protocols.Count; i++)
		{
			...
			try
			{
				...
				else
				{
					...
					result.ExchangeAlgorithm = string.Format("{0}", stream.KeyExchangeAlgorithm).ToUpper();
					if ((int)stream.KeyExchangeAlgorithm == 44550)
						result.ExchangeAlgorithm = "ECDH";
					result.CipherAlgorithm = string.Format("{0}", stream.CipherAlgorithm).ToUpper();
					result.HashAlgorithm = string.Format("{0}", stream.HashAlgorithm).ToUpper();
					status = true;
				}
			}
			catch (Exception ex1)
	...
	*/
	...
	{
		result.Certificate = new System.Security.Cryptography.X509Certificates.X509Certificate2(certificate);
		result.SslPolicyErrors = sslPolicyErrors;
